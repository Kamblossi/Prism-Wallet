...
if (isset($input['avatar'])) {
    ...
    if ($avatar === $userAvatar) {
        ...
        if (storage_driver() === 'supabase') {
            $deleted = storage_delete('logos/avatars/' . basename($cleanAvatar));
        } else {
        ...
        echo json_encode(array("success" => $deleted, "message" => $deleted ? translate("success", $i18n) : translate("error", $i18n)));
    }
} else {
