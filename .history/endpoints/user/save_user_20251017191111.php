...
// No local directories needed when using remote storage; temp files will be used
...
function resizeAndUploadAvatar($uploadedFile, $name)
{
    $targetWidth = 80;
    $targetHeight = 80;
    ...
    if (move_uploaded_file($uploadedFile['tmp_name'], $tmpPath)) {
        ...
        if ($fileInfo !== false) {
            ...
            $tmpOut = tempnam(sys_get_temp_dir(), 'avatar_out');
            if ($fileExtension === 'png') {
                imagepng($resizedImage, $tmpOut);
            } elseif ($fileExtension === 'jpg' || $fileExtension === 'jpeg') {
                imagejpeg($resizedImage, $tmpOut);
            } elseif ($fileExtension === 'gif') {
                imagegif($resizedImage, $tmpOut);
            } elseif ($fileExtension === 'webp') {
                imagewebp($resizedImage, $tmpOut);
            } else {
                return "";
            }
            ...
            // push to storage
            $relative = 'logos/avatars/' . $fileName;
            storage_put_file($relative, $tmpOut, mime_content_type($tmpOut) ?: null);
            @unlink($tmpOut);
            // Store path consistent with previous schema: for local driver keep relative path, for supabase store the same filename and resolve via helper on render
            return (storage_driver() === 'supabase') ? $fileName : ("images/uploads/logos/avatars/" . $fileName);
        }
    }
...
if (
    ...
    $name = $file['name'];
    $avatar = resizeAndUploadAvatar($_FILES['profile_pic'], $name);
    }
