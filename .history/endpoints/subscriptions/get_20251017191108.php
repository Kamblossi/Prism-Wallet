...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true) {
  ...
  foreach ($subscriptions as $subscription) {
    ...
    $id = $subscription['id'];
    $print[$id]['id'] = $id;
    // Build subscription logo URL respecting storage driver
    if (!empty($subscription['logo'])) {
      if (storage_driver() === 'supabase') {
        $print[$id]['logo'] = storage_public_url('logos/' . basename($subscription['logo']));
      } else {
        // Legacy/local path
        $basenameLogo = basename($subscription['logo']);
        $print[$id]['logo'] = 'images/uploads/logos/' . $basenameLogo;
      }
    } else {
      $print[$id]['logo'] = '';
    }
    ...
    // Build payment method icon URL (built-ins live under images/uploads/icons, customs under storage logos)
    $pmIconVal = $payment_methods[$paymentMethodId]['icon'] ?? '';
    if (strpos($pmIconVal, 'images/uploads/icons/') === 0) {
      $print[$id]['payment_method_icon'] = $pmIconVal; // built-in icon
    } else {
      $pmIconBase = basename($pmIconVal);
      $print[$id]['payment_method_icon'] = (storage_driver() === 'supabase')
        ? storage_public_url('logos/' . $pmIconBase)
        : ('images/uploads/logos/' . $pmIconBase);
    }
