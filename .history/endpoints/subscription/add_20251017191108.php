...
// Using storage abstraction (local or supabase) — no local dir creation needed here.
...
function getLogoFromUrl($url, $name, $settings, $i18n)
{
    ...
    if ($imageData !== false) {
        ...
        if (saveLogo($imageData, $relativePath, $name, $settings)) {
            curl_close($ch);
            return $fileName;
        } else {
...
function saveLogo($imageData, $relativePath, $name, $settings)
{
    ...
    if ($image !== false) {
        ...
        if (extension_loaded('imagick')) {
            ...
            storage_put_file($relativePath, $tmpOut, 'image/png');
            @unlink($tmpOut);
...
function resizeAndUploadLogo($uploadedFile, $name, $settings)
{
    $targetWidth = 135;
    $targetHeight = 42;
...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true) {
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        if ($logoUrl !== "") {
            $logoFromUrl = getLogoFromUrl($logoUrl, $name, $settings, $i18n);
            ...
            $uploadedLogo = resizeAndUploadLogo($_FILES['logo'], $name, $settings);
