...
// Create an IntlDateFormatter only if the intl extension is available
...
// Resolve avatar URL for storage-backed uploads when needed
$avatarUrl = $userData['avatar'] ?? 'images/avatars/0.svg';
if (!empty($avatarUrl) && strpos($avatarUrl, '/') === false && storage_driver() === 'supabase') {
  $avatarUrl = storage_public_url('logos/avatars/' . basename($avatarUrl));
}
      ...
      <nav>
        <div class="dropdown">
          <button class="dropbtn" onClick="toggleDropdown()">
            <img src="<?= htmlspecialchars($avatarUrl, ENT_QUOTES, 'UTF-8') ?>" alt="me" id="avatar">
