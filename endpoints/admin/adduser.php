...
// Include world currencies configuration
...
// Get all world currencies for admin user creation
$currencies = getCurrenciesForRegistration();
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $pdo->prepare('SELECT main_currency, language FROM users WHERE id = :id');
    ...
    $stmt = $pdo->prepare('SELECT code FROM currencies WHERE id = :id');
    $stmt->bindValue(':id', $currency, PDO::PARAM_STR);
    $row = $stmt->execute();
    $main_currency = $row->fetchArray()['code'];

    $query = "INSERT INTO userss (username, email, password, main_currency, avatar, language, budget) VALUES (:username, :email, :password, :main_currency, :avatar, :language, :budget)";
    ...
    // PDO conversion - removed result check
        ...
        if ($newUserId > 1) {
            ...
            $query = "UPDATE users SET main_currency = :main_currency WHERE id = :user_id";
            $stmt = $pdo->prepare($query);
